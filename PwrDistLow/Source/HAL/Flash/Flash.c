 ...
 *
 *****************************************************************************/
BOOL Flash_Initialize( void )
{
  // initialize the flash 
  U32 uTemp;
  
  // build the value
  uTemp =  ( g_tFlashDef.bPrefetchEnb )  ? FLASH_ACR_PRFTEN : 0;
  uTemp |= ( g_tFlashDef.bInstCacheEnb ) ? FLASH_ACR_ICEN   : 0;
  uTemp |= ( g_tFlashDef.bDataCacheEnb ) ? FLASH_ACR_DCEN   : 0;
  uTemp |= BITS( g_tFlashDef.eWaitStates, FLASH_ACR_LATENCY_Pos );
  FLASH->ACR = uTemp;
