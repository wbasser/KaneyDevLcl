...
BOOL LedHT16K33Handler_Initialize( void )
{
  ...
  if (( bStatus = LedHT16K33Handler_LocalInitialize( )) == FALSE )
  {
    // for each display
    for( eEnum = 0; eEnum < LEDHT16K33_ENUM_MAX; eEnum++ )
    {
      ...
      // set the display
      LedHT16K33Handler_SetBrightness( eEnum, LEDHT16K33_BRIGHTNESS_13 );
    }
  }
 ...
 * @param[in]   bUpdEnable  enable update
 *
 *****************************************************************************/
void LedHT16K33Handler_DisplayColon( U8 nDeviceID, BOOL bDisplayColon, BOOL bUpdEnable )
{
  // check for colon
  ...
  {
    // turn off colon
    anDisplayBuffer[nDeviceID ][ g_anLedHT16K33CharOffset[ LEDHT16K33_OFFSET_COLON ]  << 1 ] = 0x00;
  }
 ...
 *
 *****************************************************************************/
void LedHT16K33Handler_DisplayChar( U8 nDeviceID, U8 nDigit, U8 nCharacter, BOOL bDecimal, BOOL bUpdEnable )
{
...
void LedHT16K33Handler_DisplayNumber( U8 nDeviceID, U8 nBase, U32 uValue, U8 nDecimalPointLoc, BOOL bDisplayColon )
{
      ...
      (( 16 == nBase ) && ( uValue <= LEDHT16K33_MAX_NUMBER_BASE_HEX )))
  {
    ...
    while ( uValue > 0 )
    {
      ...
      // display the digit
      LedHT16K33Handler_DisplayChar( nDeviceID, nCurDigit, nCurValue, ( nDecimalPointLoc == nCurDigit ), FALSE);
  ...
  else
  {
    ...
    {
      // display a dash
      LedHT16K33Handler_DisplayChar( nDeviceID, nCurDigit, LEDHT16K33_SPCCHAR_MINUS, OFF, FALSE );
    }
  }
