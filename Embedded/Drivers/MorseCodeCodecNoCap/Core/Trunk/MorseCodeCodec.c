...
static  U16               wCurRcvCounts;
static  U8                nCurRcvWpm;
static  FILTAVERAGECTL    tRcvDotFilter;
static  MOFSECODEDECDEF   aptCodeHash[ CODE_TABLE_LENGTH ];

// declare the buffer
FILTAVERAGESTORAGE( RcvDot, RCVDLEM_AVERAGE_SIZE );

// local function prototypes --------------------------------------------------
static  void  CreateHashLookupTable( void );
...
const CODE MORSEECODECODEDEF  atCodes[ CODE_TABLE_LENGTH ] =
{
  ...
  { 0x00, { 0x00, 0x00 }},   // 0x21 - no code
  { 0x00, { 0x00, 0x00 }},   // 0x22 - no code
  { 0x00, { 0x00, 0x00 }},   // 0x23 - no code
  { '$',  { 0x07, 0x09 }},   // 0x24 - $
  { 0x00, { 0x00, 0x00 }},   // 0x25 - no code
  { 0x00, { 0x00, 0x00 }},   // 0x26 - no code
  { '\'', { 0x06, 0x33 }},   // 0x27 - '
  { 0x00, { 0x00, 0x00 }},   // 0x28 - no code
  { 0x00, { 0x00, 0x00 }},   // 0x29 - no code
  { '*',  { 0x06, 0x05 }},   // 0x2A - *
  { '+',  { 0x05, 0x0A }},   // 0x2B - +
  { '-',  { 0x06, 0x1E }},   // 0x2C - ,
  { '-',  { 0x05, 0x11 }},   // 0x2D - -
  { '.',  { 0x06, 0x38 }},   // 0x2E - .
  { 0x00, { 0x00, 0x00 }},   // 0x2F - no code
  { '0',  { 0x05, 0x1F }},   // 0x30 - 0
  { '1',  { 0x05, 0x0F }},   // 0x31 - 1
  { '2',  { 0x05, 0x07 }},   // 0x32 - 2
  { '3',  { 0x05, 0x03 }},   // 0x33 - 3
  ...
  { '9',  { 0x05, 0x1E }},   // 0x39 - 9
  { ':',  { 0x06, 0x0C }},   // 0x3A - :
  { 0x00, { 0x00, 0x00 }},   // 0x3B - no code
  { 0x00, { 0x00, 0x00 }},   // 0x3C - no code
  { 0x00, { 0x00, 0x00 }},   // 0x3D - no code
  { 0x00, { 0x00, 0x00 }},   // 0x3E - no code
  { '?',  { 0x05, 0x01 }},   // 0x3F - ?
  { 0x00, { 0x00, 0x00 }},   // 0x40 - no code
  { 'A',  { 0x02, 0x01 }},   // 0x41 - A
  { 'B',  { 0x04, 0x08 }},   // 0x42 - B
  { 'C',  { 0x04, 0x0A }},   // 0x43 - C
  { 'D',  { 0x03, 0x04 }},   // 0x44 - D
...
BOOL MorseCodeCodec_Initialize( void )
{
  ...
  bStatus = MorseCodeCodec_LocalInitialize( );
  ...
  // create the code hash table
  memset( aptCodeHash, 0, sizeof( aptCodeHash ));
  CreateHashLookupTable( );
...
static void CreateHashLookupTable( void )
{
  ...
  for ( nIndex = 0; nIndex < CODE_TABLE_LENGTH; nIndex++ )
  {
    ...
    // check for a invalid - entry
    if (( ptEntry->wValue != 0xFFFF ) && ( ptEntry->wValue != 0x00000 )) 
    {
      // create the hash value
      nKeyHash = ptEntry->wValue %= CODE_TABLE_LENGTH;
      
      // insert the entry into the table
      while( aptCodeHash[ nKeyHash ] != NULL )
      {
        // index to the next
        nKeyHash++;
        nKeyHash %= CODE_TABLE_LENGTH;
      }
      
      // insert item
      aptCodeHash[ nKeyHash ] = ptEntry;
    }
  }
}
...
static C8 CodeToAscii( PMORSEECODECODEDEF ptCode, BOOL bEnableSpace )
{
  ...
  {
    // create the hash cod
    nHash = ptCode->wValue % CODE_TABLE_LENGTH;
    
    // now search
    while( aptCodeHash[ nHash ] != NULL )
    {
      // check for a math
      if ( aptCodeHash[ nHash ]->wValue == ptCode->wValue )
      {
        // set the character
        cChar = aptCodeHash[ nHash ]->nCode;
        break;
      }
      else
      {
        nHash++;
        nHash %= CODE_TABLE_LENGTH;
      }
    }
  }
