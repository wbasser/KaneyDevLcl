...
// Macros and Defines ---------------------------------------------------------
/// define the macro for instatiating the alarm entry
#define ALARMDEFM( ll, lo, hi, hh, llenab, loenab, hienab, hhenab ) \
  { \
    .xLoLoThreshold = ll, \
    .xLoThreshold   = lo, \
    .xHiThreshold   = hi, \
    .xHiHiThreshold = hh, \
    .tEnableFlags.bLLEnable = llenab, \
    .tEnableFlags.bLoEnable = loenab, \
    .tEnableFlags.bHiEnable = hienab, \
    .tEnableFlags.bHHEnable = hhenab, \
  }
...
typedef struct _ALARMDEF
{
  ...
  struct
  {
    BOOL  bLLEnable;          ///< enable lo-lo alarm
    BOOL  bLoEnable;            ///< enable lo alarm
    BOOL  bHiEnable;            ///< enable hi alarm
    BOOL  bHHEnable;          ///< enable hi-hi alarm
  } tEnableFlags;
} ALARMDEF, *PALARMDEF;
