...
//  {
//    // post the event
//    DALIBusCommissioner_PostEvent( tState );
//
...
static void CfbStateFindShortAddrEnt( void )
{
  ...
  DALIBusCommissioner_PostEvent( DALIBUSCOM_EXEC_EVENT );
}
...
static void CfbStateSendProgAddrEnt( void )
{
  ...
  if ( nCurDevIdx >=  DALI_MAX_NUM_OF_DEVICES )
  {
    ...
    // force an entry
    DALIBusCommissioner_PostPriorityEvent( DALIBUSCOM_EXEC_EVENT );
  }
  else
  {
...
static void CfbStateSendTerminateEnt( void )
{
  ...
  // force an exit
  DALIBusCommissioner_PostPriorityEvent( DALIBUSCOM_EXIT_COMMISSION );
}
