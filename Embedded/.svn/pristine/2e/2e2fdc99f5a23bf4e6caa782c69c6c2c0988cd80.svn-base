...
#define XMIT_WAIT_TIME_ECHO                     ( 22 )
#define INTERFRAME_WAIT_TIME                    ( 12 )
#define RECV_WAIT_TIME                          ( 16 )
#define BACKFRM_WAIT_TIME                       (  9 )
...
static U8 CtlStateWaitExec( STATEEXECENGARG xArg )
{
  ...
  switch( xArg )
  {
    case DALIBUSMASTER_CTLEVENT_DONE :
      // post the response
      DALIBusMaster_QueuePutRcvMsg( &tCurMessage );
...
static void XmtStateIdleExt( void )
{
  ...
  // start a timer
  DALIBusMaster_XmtTaskTimer( ON, XMIT_WAIT_TIME_ECHO );
}
...
static void XmtStateWaitRcvDoneEnt( void )
{
  ...
  // start a timer
  DALIBusMaster_XmtTaskTimer( ON, RECV_WAIT_TIME );
}
...
{
  // start a timer
  DALIBusMaster_XmtTaskTimer( ON, INTERFRAME_WAIT_TIME );
}
...
{
  // start a timer
  DALIBusMaster_XmtTaskTimer( ON, BACKFRM_WAIT_TIME );
}
