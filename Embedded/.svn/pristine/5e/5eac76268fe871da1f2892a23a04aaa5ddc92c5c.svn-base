 ...
 *
 *****************************************************************************/
BOOL RedirectionManager_Initialize( void )
{
  REDIRECTENUM  eDevIdx;
  BOOL          bStatus = FALSE;

  // call the local initilization
  bStatus = RedirectionManager_LocalInitialize( );
...
BOOL RedirectionManager_SetRedirect( REDIRECTENUM eSrc, REDIRECTENUM eDst )
{
  ...
  if (( eSrc < REDIRECT_ENUM_MAX ) && ( eDst < REDIRECT_ENUM_MAX ) && ( eSrc < eDst ) && ( aeDestinations[ eSrc ] == REDIRECT_ENUM_ILLEGAL ))
  {
    ...
    // call the redirection update function
    RedirectManager_UpdateRedirectStatus(( nRedirectCount != 0 ));
...
BOOL RedirectionManager_RedirectChar( REDIRECTENUM eSrc, U8 nChar )
{
   ...
   *
   * @brief redirect command handler
   *
   * This function provides the redirect command handler
   *
