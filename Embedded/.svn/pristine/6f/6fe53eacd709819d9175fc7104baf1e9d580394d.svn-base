 ...
 *
 *****************************************************************************/
BOOL AsciiCommandHandler_Initialize( void )
{
  BOOL        bStatus = FALSE;
  ASCCMDENUM  eIdx;
  PASCCMDDEF  ptDef;
  ...
  if ( eProtEnum < ASCCMD_ENUM_MAX )
  {
    ...
    else if( cRcvChar == '\b' ) // check for a backspae
    {
      ...
      if( ptCtl->nBufIdx > 0 )
      {
        ...
        nIdx = ( bEchoFlag ^ ptCtl->bForceEcho ) ? 1 : 0;
...
/******************************************************************************
 * @function AsciiCommandHandler_EchoControl
 *
 * @brief echo control
 *
 * This function will set the echo control
 *
 * @param[in]   eProtEnum       protocol enumeration
 * @param[in]   bEchoState      echo state
 *
 * @return      appropriate status
 *
 *****************************************************************************/
ASCCMDSTS AsciiCommandHandler_EchoControl( ASCCMDENUM eProtEnum, BOOL bEchoState )
{
  ASCCMDSTS eStatus = ASCCMD_STS_NONE;
  ...
  if ( eProtEnum < ASCCMD_ENUM_MAX )
  {
    // set the force echo flag state
    atLclCtl[ eProtEnum ].bForceEcho = bEchoState;
  }
  else
  {
    // set the error status
