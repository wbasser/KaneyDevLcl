...
static void ConfigureOscDfll( PCLOCKOSCDEF ptDef )
{
  ...
  tMult.reg = SYSCTRL_DFLLMUL_MUL( ptDef->wDfllMulitplier );
  tMult.reg |= SYSCTRL_DFLLMUL_CSTEP( ptDef->nDfllCoarseStep >> 1 );
  tMult.reg |= SYSCTRL_DFLLMUL_FSTEP( ptDef->wDfllFineStep >> 1 );
  ...
  #endif
  tCtrl.bit.QLDIS = OFF;
  tCtrl.bit.CCDIS = ( ptDef->tFlags.bDisableChillCycle ) ? FALSE : TRUE;
