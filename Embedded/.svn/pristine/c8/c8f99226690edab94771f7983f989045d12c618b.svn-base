...
#if (( BUTTONMANAGER_ARG_SIZE_BYTES == 1 ) && ( BTNMNGR_ENUM_MAX > 31 ))
{
 ...
 *****************************************************************************/
BOOL ButtonManager_Initialize( void )
{
  BOOL  bStatus = FALSE;
  
  // get a copy of the config block
  MEMCPY_P( &tConfig, &g_tBtnMgrCfg, BTNMNGRCFG_SIZE );
...
{ 
  ...
  // copy the def
  MEMCPY_P( &tBtnDef, &g_atBtnMgrDefs[ eBtnEnum ], BTNMNGRDEF_SIZE );
...
void ButtonManager_Process( void )
{
  ...
  for ( eBtn = 0; eBtn < BTNMNGR_ENUM_MAX; eBtn++ )
  {
    ...
    // copy the def
    MEMCPY_P( &tBtnDef, &g_atBtnMgrDefs[ eBtn ], BTNMNGRDEF_SIZE );

    // get the current state
    if ( tBtnDef.pvGetStatus != NULL )
    {
      // get the status
      bKeyState = tBtnDef.pvGetStatus( tBtnDef.uKeyEnum );
    }
    else
    {
      ...
      ( ptDef->tEventFlags.bLongHoldEnable   && ( eEvent == BTNMNGR_EVENT_LONGHOLD )))
  {
    ...
    switch( ptDef->eRptMethod )
    {
      ...
      case BTNMNGR_RPTMETHOD_EVENT :
        // post it
        ButtonManager_PostEvent( ptDef, eEvent, eKey );
        break;
