...
static  void        StuffXmtData( PLCLBUFCTL ptBufCtl, U8 nData, BOOL bEscapeEnb, BOOL bEnableChk );
 ...
 *
*****************************************************************************/
BOOL BinaryCommandHandler_Initialize( void )
{
  BOOL        bStatus = FALSE;
    ...
    if ( ptLclCtl->tStateCtl.nCurState < RCV_STATE_CRCM )
    {
      ...
      switch( ptLclCtl->tRcvBuffer.eCheckMode )
      {
        ...
        case BINCMD_CHECKMODE_CRC :
          ptLclCtl->tRcvBuffer.tCheck.wValue = BinaryCommandHandler_ComputeCrcByte( ptLclCtl->tRcvBuffer.tCheck.wValue, nRcvChar );
          break;
...
BINCMDSTS BinaryCommandHandler_BeginMessage( BINCMDENUM eProtEnum, U8 nCommand, U8 nOption1, U8 nOption2 )
{
  ...
  if ( eProtEnum < BINCMD_ENUM_MAX )
  {
    ...
    switch( ptBufCtl->eCheckMode )
    {
      ...
      case BINCMD_CHECKMODE_CRC :
        ptBufCtl->tCheck.wValue = BinaryCommandHandler_GetInitialValue( );
        break;
    ...
    // now add the header
    StuffXmtData( ptBufCtl, CH_DLE, FALSE, TRUE );
    StuffXmtData( ptBufCtl, CH_SOH, FALSE, TRUE );
    
    // test for multidrop
    if ( PGM_RDBYTE( ptLclDef->bMultiDropMode ) == TRUE)
    {
      // send the destination/source address's
      StuffXmtData( ptBufCtl, ptBufCtl->nSrcDstAddr, TRUE, TRUE );
      StuffXmtData( ptBufCtl, ptLclCtl->nLclAddr, TRUE, TRUE );
    }
    
    // send the command/option bytes
    StuffXmtData( ptBufCtl, nCommand, TRUE, TRUE );
    StuffXmtData( ptBufCtl, nOption1, TRUE, TRUE );
    StuffXmtData( ptBufCtl, nOption2, TRUE, TRUE );

    // check for sequence numbers
    if ( PGM_RDBYTE( ptLclDef->bSequenceEnable ))
    {
      // send the sequence number
      StuffXmtData( ptBufCtl, ptBufCtl->nSequence, TRUE, TRUE );
    }
...
BINCMDSTS BinaryCommandHandler_SetMessageByte( BINCMDENUM eProtEnum, U8 nXmtChar )
{
  ...
  if ( eProtEnum < BINCMD_ENUM_MAX )
  {
    ...
    if ( ptLclCtl->tXmtBuffer.bDataBlock == FALSE )
    {
      ...
      StuffXmtData( &ptLclCtl->tXmtBuffer, CH_DLE, FALSE, TRUE );
      StuffXmtData( &ptLclCtl->tXmtBuffer, CH_STX, FALSE, TRUE );
    }
    
    // now stuff the data
    StuffXmtData( &ptLclCtl->tXmtBuffer, nXmtChar, TRUE, TRUE );
  }
...
BINCMDSTS BinaryCommandHandler_SetMessageBlock( BINCMDENUM eProtEnum, PU8 pnData, U16 wLength )
{
  ...
  if ( eProtEnum < BINCMD_ENUM_MAX )
  {
    ...
    if ( ptBufCtl->bDataBlock == FALSE )
    {
      ...
      // stuff the data header
      StuffXmtData( ptBufCtl, CH_DLE, FALSE, TRUE );
      StuffXmtData( ptBufCtl, CH_STX, FALSE, TRUE );
    }
    ...
    {
      // stuff the data
      StuffXmtData( ptBufCtl, *( pnData++ ), TRUE, TRUE );
    }
...
BINCMDSTS BinaryCommandHandler_SendMessage( BINCMDENUM eProtEnum )
{
  ...
  if ( eProtEnum < BINCMD_ENUM_MAX )
  {
    ...
    // send the trailer
    StuffXmtData( ptBufCtl, CH_DLE, FALSE, TRUE );
    StuffXmtData( ptBufCtl, CH_EOT, FALSE, TRUE );
    ...
    switch( ptBufCtl->eCheckMode )
    {
      case BINCMD_CHECKMODE_EOR :
        // send the character
        StuffXmtData( ptBufCtl, ptBufCtl->tCheck.anValue[ LE_U16_LSB_IDX ], FALSE, TRUE );
        break;
      ...
      case BINCMD_CHECKMODE_CRC :
        ...
        StuffXmtData( ptBufCtl, ptBufCtl->tCheck.anValue[ LE_U16_MSB_IDX ], FALSE, FALSE );
        StuffXmtData( ptBufCtl, ptBufCtl->tCheck.anValue[ LE_U16_LSB_IDX ], FALSE, FALSE );
        break;
...
static U8 RcvStateHdr1Exc( STATEEXECENGARG xArg )
{
  ...
  if ( xArg == CH_SOH )
  {
    ...
    switch( PGM_RDBYTE( ptLclDef->eCheckMode ))
    {
      ...
      case BINCMD_CHECKMODE_CRC :
        ptLclCtl->tRcvBuffer.tCheck.wValue = BinaryCommandHandler_GetInitialValue( );
        ptLclCtl->tRcvBuffer.tCheck.wValue = BinaryCommandHandler_ComputeCrcByte( ptLclCtl->tRcvBuffer.tCheck.wValue, CH_DLE );
        ptLclCtl->tRcvBuffer.tCheck.wValue = BinaryCommandHandler_ComputeCrcByte( ptLclCtl->tRcvBuffer.tCheck.wValue, CH_SOH );
        break;
...
{
  // store the value
  ptLclCtl->tRcvActualCheck.anValue[ LE_U16_LSB_IDX ] = xArg;
 ...
 * @param[in]   bEnableChk  enable the adding of the check character
 *
 *****************************************************************************/
static void StuffXmtData( PLCLBUFCTL ptBufCtl, U8 nData, BOOL bEscapeEnb, BOOL bEnableChk  )
{
  ...
  if ( bEnableChk )
  {
    ...
    switch( ptBufCtl->eCheckMode )
    {
      ...
      case BINCMD_CHECKMODE_CRC :
        ptBufCtl->tCheck.wValue = BinaryCommandHandler_ComputeCrcByte( ptBufCtl->tCheck.wValue, nData );
        break;
  ...
  *( ptBufCtl->pnBuffer + ptBufCtl->wIndex++ ) = nData;
  ...
  if (( bEscapeEnb ) && ( nData == CH_DLE ))
  {
    ...
    StuffXmtData( ptBufCtl, CH_SPC, FALSE, bEnableChk ); 
  }
}
