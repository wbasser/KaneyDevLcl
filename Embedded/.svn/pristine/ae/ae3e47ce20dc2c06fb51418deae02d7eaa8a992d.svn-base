...
static const ALIGNED4 USBDEVDESCRIPTOR tUsbDevDescriptor =
{
  .tHeader                =
  {
  ...
  .wVendor                = USBCDCHAND_VENDOR_ID,
  .wProduct               = USBCDCHAND_PRODUCT_ID,
  .wBcdDevice             = USB_VERSION_BCD( 0, 0, 1 ),
  .nManufacturer          = USBCDCHAND_STRING_MANUF,
  .nProduct               = USBCDCHAND_STRING_PROD,
  .nSerialNumber          = USBCDCHAND_STRING_SERNUM,
  .nNumConfigurations     = 1,
};

static const ALIGNED4 USBCONFIGURATION tUsbConfiguration =
{
  // configuration descriptor
  .tCfgDescriptor         =
  {
...
const ALIGNED4 USBSTRINGZERODESCRIPTOR  tUsbStringZeroDescriptor =
{
  .tHeader              =
  {
...
static void CdcEnumerate( void )
{
  VU8       nRequestType, nRequest, nDir, nDesc, nStrSize, nStrIndex;
  ...
  nDesc = anUsbEpBufOut[ LCLBUF_IDX_CTL ][ 3 ];
  ...
  switch( MAKEU16( nRequest, nRequestType ))
  {
    case STD_GET_DESCRIPTOR :
      ...
      switch( nDesc )
      {
        case USB_DESCTYPES_DEVICE :
          LclSendData( USB_ENDPOINT_CTL, ( PU8 )&tUsbDevDescriptor, MIN( USBDEVDESCRIPTOR_SIZE, wLength ));
          break;

        case USB_DESCTYPES_CONFIGURATION :
          LclSendData( USB_ENDPOINT_CTL, ( PU8 )&tUsbConfiguration, MIN( USBCONFIGURATION_SIZE, wLength ));
          break;

        case USB_DESCTYPES_STRING :
          // get teh string index
          nStrIndex = wValue & 0xFF;

          // is the  language desciptor
          if ( nStrIndex == 0 )
          {
            LclSendData( USB_ENDPOINT_CTL, ( PU8 )&tUsbStringZeroDescriptor, MIN( tUsbStringZeroDescriptor.tHeader.nLength, wLength ));
          }
          else if ( nStrIndex < USBCDCHAND_STRING_MAX )
          {
            // allocate space for the string
            nStrSize = USBDESCHEADR_SIZE + ( wcslen( g_awStrings[ nStrIndex ] ) * 2 );
            pnOutputDesc = malloc( nStrSize );
            *( pnOutputDesc + 0 ) = nStrSize;
            *( pnOutputDesc+ 1 ) = USB_DESCTYPES_STRING;

            // now copy the string
            pwTemp = ( wchar_t* )g_awStrings[ nStrIndex ];
            wcscpy(( wchar_t* )( pnOutputDesc + 2 ), pwTemp );

            // send it
            wLength = MIN( wLength, nStrSize ); 
            LclSendData( USB_ENDPOINT_CTL, ( PU8 )pnOutputDesc, wLength );
    ...
    case SET_LINE_CODING :
      // copy the data
      //memcpy( &tNewCoding, &anUsbEpBufOut[ LCLBUF_IDX_CTL ][ 8 ], USBCDCLINECODING_SIZE );
