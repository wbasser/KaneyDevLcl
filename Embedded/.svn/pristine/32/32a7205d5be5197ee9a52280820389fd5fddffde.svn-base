...
void LedManager_ProcessScan( void )
{
  ...
  static void ChangeLedState(PLEDDEF ptDef, BOOL bState, U8 nIntensity)
  {
    ...
    switch( ptDef->eDriveType )
    {
      ...
      case LED_DRIVETYPE_SPECIAL :
        ptDef->tRowDrive.tFuncs.pvSpclFunc( ptDef->nColIndex, bState );
        break;
      ...
      case LED_DRIVETYPE_PWMSPC :
        ptDef->tRowDrive.tFuncs.pvSpclPwmFunc( ptDef->tRowDrive.eDrivePin, ptDef->nColIndex, nIntensity );
        break;
  ...
  static  ASCCMDSTS CmdSetLed( U8 nCmdEnum )
  {
    ...
    // set the led
    #if ( LEDMANAGER_ENABLE_DIMMABLE_LEDS == ON )
      eError = LedManager_Control( eLedSel, eAction, 0, tValue.anValue[ LE_U32_LSB_IDX ] );
    #else
    ...
    static  ASCCMDSTS CmdSetRgb( U8 nCmdEnum )
    {
      ...
      AsciiCommandHandler_GetValue( nCmdEnum, 2, &tValue.uValue );
        ...
        AsciiCommandHandler_GetValue( nCmdEnum, 5, &tValue.uValue );
        nIntBlu = tValue.anValue[ LE_U32_LSB_IDX ];
      #endif // LEDMANAGER_ENABLE_DIMMABLE_LEDS
      
      // set the led  
      #if ( LEDMANAGER_ENABLE_DIMMABLE_LEDS == ON )
        eError = LedManager_RgbControl( eLedSel, eAction, 0,  0, nIntRed, nIntGrn, nIntBlu );
      #else
