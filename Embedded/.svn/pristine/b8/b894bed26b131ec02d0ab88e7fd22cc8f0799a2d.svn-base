...
static  PVDSNCALLBACK         pvLclDnsCallback;
...
BOOL MLAWINC1500Handler_Initialize( void )
{
  ...
  pvLclDnsCallback = NULL;
  pvLclErrCallback = NULL;
  ...
  if ( ON == ptCurCtl->bAutoSocket )
  {
    ...
    switch( eEventCode )
    {
      case M2M_SOCKET_BIND_EVENT :
        ...
        if ( ptEventData->bindStatus == 0 )
        {
          ...
          else
          {
            ...
            ptCurCtl->pvGetRcvBuffer( cSock, &ptCurCtl->pvCallback, &ptCurCtl->pnRcvBuffer, &ptCurCtl->wBufSize );
      ...
      case M2M_SOCKET_DNS_RESOLVE_EVENT :
        // check for a null
        if ( pvLclDnsCallback != NULL )
        {
          // call it
          pvLclDnsCallback( ptEventData->dnsReply.hostIp );
        }
        break;
      ...
      case M2M_SOCKET_RECVFROM_EVENT :
        ...
        if ( ptEventData->recvMsg.bufSize > 0 )
        {
          ...
          // check for receive from
          if ( M2M_SOCKET_RECVFROM_EVENT == eEventCode )
          {
            // setup a receive
            recvfrom( cSock, ptCurCtl->pnRcvBuffer, ptCurCtl->wBufSize, 0 );
          }
...
SOCKET MLAWINC1500Handler_OpenSock( BOOL bAutoSocket, IPADDR tAddr, U16 wPort, MLAWINC1500HANDSKTTYPE eUdpTcp, MLAWINC1500HANDCLTSVR eClientSvr, PVSKTRCVCALLBACK pvCallback, PVSERVACPTCALLBACK pvGetRcvBuffer )
{
  ...
  if (( cSocket = socket( AF_INET, eUdpTcp, 0 )) > -1 )
  {
    ...
    // only bind/connect if auto socket
    // check for server
...
/******************************************************************************
 * @function MLAWINC1500Handler_GetHostByName
 *
 * @brief get host by name
 *
 * This function will send data
 *
 * @param[in]   pszHostName     host name
 * @param[in]   pvDnsCallback   pointer to the callback function
 *****************************************************************************/
void MLAWINC1500Handler_GetHostByName( PC8 pszHostName, PVDSNCALLBACK pvDnsCallback )
{
  // store the callback
  pvLclDnsCallback = pvDnsCallback;
