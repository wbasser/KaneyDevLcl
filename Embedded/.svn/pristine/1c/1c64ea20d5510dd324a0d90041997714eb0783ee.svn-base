...
/// define the helper macro for creating a PWM drive led entry
#define LEDDEF_DIMMEDSPC( func, device, chan ) \
  { \
    .eDriveType   = LED_DRIVETYPE_PWMSPC, \
    .tRowDrive    = \
    { \
      .eDrivePin  = device, \
      .tFuncs = \
...
typedef enum _LEDDRIVETYPE
{
  ...
  LED_DRIVETYPE_PWM,        ///< PWM type
  LED_DRIVETYPE_PWMSPC,     ///< PWM Type with special call
  LED_DRIVETYPE_MAX
} LEDDRIVETYPE;
...
typedef void ( *PVLEDSPCPWMFUNC )( U8, U8, U8 );
...
typedef struct _LEDDEF
{
  ...
  union
  {
    ...
    union
    {
      ...
      PVLEDSPCPWMFUNC pvSpclPwmFunc;  ///< special drive function for PWMS
    } tFuncs;
  } tRowDrive;                    ///< row drive
