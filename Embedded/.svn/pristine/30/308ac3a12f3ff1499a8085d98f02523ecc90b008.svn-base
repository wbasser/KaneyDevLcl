...
#if (( EEPROMHANDLER_ENABLE_EMULATION == ON ) || ( EEPROMHANDLER_ENABLE_BACKGROUND_WRITES == ON ))
...
// command handlers
  ...
  static  ASCCMDSTS CmdQryEep( U8 nCmdEnum );
...
#if (( EEPROMHANDLER_ENABLE_BACKGROUND_WRITES == ON ) || ( EEPROMHANDLER_ENABLE_EMULATION == OFF ))
...
/// define the command strings
  ...
  static  const CODE C8 szQryEep[ ]   = { "QRYEEP" };

  /// string declarations
  static  const CODE C8 szRspRep[ ]   = { "RREP, 0x%04X, 0x%02X\n\r" };
  static  const CODE C8 szRspEep[ ]   = { "RSTS, Oper: %s, Size: %dK\n\r" };
  static  const CODE C8 szStsEmu[ ]   = { "Emulation" };
  static  const CODE C8 szStsAct[ ]   = { "Actual" };

  /// initialize the command table
  const CODE ASCCMDENTRY g_atEepromDbgHandlerTable[ ] =
  {
    ASCCMD_ENTRY( szWrtEep, 4, 2, ASCFLAG_COMPARE_GE,   EEPROMHANDLER_DIAGMODE_ENUM, CmdWrtEep ),
    ASCCMD_ENTRY( szRedEep, 4, 1, ASCFLAG_COMPARE_GE,   EEPROMHANDLER_DIAGMODE_ENUM, CmdRedEep ),
    ASCCMD_ENTRY( szDmpEep, 4, 2, ASCFLAG_COMPARE_GE,   EEPROMHANDLER_DIAGMODE_ENUM, CmdDmpEep ),
    ASCCMD_ENTRY( szFilEep, 4, 3, ASCFLAG_COMPARE_GE,   EEPROMHANDLER_DIAGMODE_ENUM, CmdFilEep ),
    ASCCMD_ENTRY( szQryEep, 6, 0, ASCFLAG_COMPARE_NONE,                           0, CmdQryEep ),
 ...
 *
 *****************************************************************************/
BOOL EepromHandler_Initialize( void )
{
  BOOL bStatus = FALSE;
  ...
  bStatus = EepromHandler_LclInitialize( );
...
EEPROMERR EepromHandler_RdBlock( U16 wAddress, U16 wLength, PU8 pnData )
{
  ...
  if(( wAddress + wLength ) < EEPROMHANDLER_DEV_SIZE )
  {
    #if (( EEPROMHANDLER_ENABLE_EMULATION == OFF ) && ( EEPROMHANDLER_ENABLE_BACKGROUND_WRITES == OFF ))
      // get the current time
      uTime = EepromHandler_GetSystemTime( ) + WAIT_FOR_BUSY_DONE_TIME;

      // wait till done
      while(( bCheckForBusyRequired = CheckForBusy( )) == TRUE )
      {
        // check for timeout
        if ( EepromHandler_GetSystemTime( ) >= uTime )
        {
...
EEPROMERR EepromHandler_WrBlock( U16 wAddress, U16 wLength, PU8 pnData )
{
  ...
  if(( wAddress + wLength ) < EEPROMHANDLER_DEV_SIZE )
  {
      ...
      // enable this task
      EepromHandler_BackgroundWriteControl( ON );
    #endif // EEPROMHANDLER_ENABLE_BACKGROUND_WRITES
  }
  else
  {
...
#if (( EEPROMHANDLER_ENABLE_EMULATION == OFF ) || ( EEPROMHANDLER_ENABLE_BACKGROUND_WRITES == ON ))
  /******************************************************************************
  ...
  static BOOL CheckForBusy( void )
  {
    ...
    {
      // call the local chkc busy
      bBusy = EepromHandler_LclCheckBusy( );
    }
    else
    {
      // clear busy
   ...
   * @function CmdQryEep
   *
   * @brief display the EEPROm satus
   *
   * This function will report actual/emulation mode and size
   *
  ...
  static ASCCMDSTS CmdQryEep( U8 nCmdEnum )
  {
    ...
    // set the mode
    pcMode = ( PCC8 )(( EEPROMHANDLER_ENABLE_EMULATION == TRUE ) ? szStsEmu : szStsAct );

    // output the error
    SPRINTF_P( pcBuffer, ( PCC8 )szRspEep, pcMode, EEPROMHANDLER_DEV_SIZE );
