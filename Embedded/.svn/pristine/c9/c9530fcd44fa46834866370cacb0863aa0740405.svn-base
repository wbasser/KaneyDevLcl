...
I2CERR I2c_Write( I2CDEVENUM eDev, PI2CXFRCTL ptXfrCtl )
{
  ...
  if ( eDev < I2C_DEV_ENUM_MAX )
  {
    ...
    while( ptI2c->SYNCBUSY.bit.SYSOP );
...
I2CERR I2c_Close( I2CDEVENUM eDev )
{
  ...
  if ( eDev < I2C_DEV_ENUM_MAX )
  {
    ...
    // enable the interrupt in the NVIC
    NVIC_DisableIRQ( SERCOM0_0_IRQn + ( g_atI2cDefs[ eDev ].eChan * 4 ));
  }
  else
  {
    // illegal device
...
#ifdef SERCOM6
  #if ( I2C_CHAN6_IN_USE == ON )
  /******************************************************************************
   * @function SERCOM6_Handler
   *
  ...
  #endif // I2C_CHAN6_IN_USE
#endif // SERCOM6

#ifdef SERCOM7
  #if ( I2C_CHAN7_IN_USE == ON )
  /******************************************************************************
   * @function SERCOM7_Handler
   *
...
static U32 ComputeBaudRate( I2CCHAN eChan, U32 uBaud )
{
  ...
  uPeripheralClock = Clock_GetPeripheralFreq( CLOCK_MUXID_SERCOM_0 + eChan );
