...
#define UART_DOUBLEBUFFER_CTRL_DEF(name, bufA, bufB, bufsize) \
    UARTDoubleBuffer s ## name = \
    { \
      .pBufA = bufA, \
      .pBufB = bufB, \
      .pActiveBuf = NULL, \
...
#define UART_ASYNC_DEF( devName, txdpad, rxdpad, baudrate, parity, wrdlen, stopbits, standby, name, rxsize, txsize, callback ) \
  { \
...
typedef struct _UARTDoubleBuffer
{
  PU8   pBufA;
  PU8   pBufB;
  U16   wBufLen;
  PU8   pActiveBuf;
  PU8   pNextBuf;
} UARTDoubleBuffer, *PUARTDoubleBuffer;
#define UARTDOUBLEBUF_SIZE    sizeof( UARTDoubleBuffer )
