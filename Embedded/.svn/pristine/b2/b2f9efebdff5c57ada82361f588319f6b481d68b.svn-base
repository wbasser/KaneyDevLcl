...
#include "StepperControlHandler/StepperControlHandler.h"
...
const CODE  SYSCTRLMNGRSCHDDEF g_atSysCtrlMngrSchdDefs[ ] = 
{
  ...
  SYSCNTRLMNGRSCHD_DEF( TASK_SCHD_ENUM_BTNMNGR,  OFF,   OFF,   OFF,   OFF,   ON,    ON,    OFF,   OFF,   OFF,   OFF,   OFF,    OFF,    OFF,    OFF,    OFF,    OFF,    NULL                          ),
  SYSCNTRLMNGRSCHD_DEF( TASK_SCHD_ENUM_LCLEVENT, OFF,   OFF,   OFF,   OFF,   ON,    ON,    OFF,   OFF,   OFF,   OFF,   OFF,    OFF,    OFF,    OFF,    OFF,    OFF,    NULL                          ),
};

#if ( TASK_TICK_ENABLE == ON )
...
/******************************************************************************
 * @function SystemControlManager_LocalHandleError
 *
 * @brief initialize the error handler
 *
 * This function will perform the error
 *
 *****************************************************************************/
void SystemControlManager_LocalHandleError( void )
{
  // for now just loop
  FOREVER
  {
...
static void LclInitEntry( void )
{
  ...
  // set the reset event
  LogHandler_AddEntry( LOG_TYPE_RESET, 0x3CC3, 0x5AA5 );

  // home the stepper
  if ( TRUE == StepperControlHandler_Home())
  {
    // go to the error state
    xStateArg = SYSCTRLMNGR_EVENT_ERRORDET;
  }
