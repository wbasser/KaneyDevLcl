...
#define PROCESS_TASK_NUMPRIEVENTS               ( 2 )
...
{
  U16   wDebounceTimeCounts;       ///< debounce time
  U16   wRepeatDelayCounts;        ///< repeat delay
  U16   wRepeatRateCounts;         ///< repeat rate
  U16   wShortHoldTimeCounts;      ///< short hold time
  U16   wMediumHoldTimeCounts;     ///< medium hold time
  U16   wLongHoldTimeCounts;       ///< long hold time
  U16   wStuckTimeCounts;          ///< stuck key time
...
BOOL ButtonManagerDynamic_Initialize( U16 wDebnMsecs, U16 wRptDlyMsecs, U16 wRptRateMsecs, U16 wHoldShrtMsecs, U16 wHoldMedMsecs, U16 wHoldLongMsecs, U16 wHoldStuckMsecs )
{
  ...
  // set the parameters
  tParams.wDebounceTimeCounts = wDebnMsecs / BTNMANAGER_PROCESS_RATE_MSECS;
  tParams.wRepeatDelayCounts = wRptDlyMsecs / BTNMANAGER_PROCESS_RATE_MSECS;
  tParams.wRepeatRateCounts = wRptRateMsecs / BTNMANAGER_PROCESS_RATE_MSECS;
  tParams.wShortHoldTimeCounts = wHoldShrtMsecs / BTNMANAGER_PROCESS_RATE_MSECS;
  tParams.wMediumHoldTimeCounts = wHoldMedMsecs / BTNMANAGER_PROCESS_RATE_MSECS;
  tParams.wLongHoldTimeCounts = wHoldLongMsecs / BTNMANAGER_PROCESS_RATE_MSECS;
  tParams.wStuckTimeCounts = wHoldStuckMsecs / BTNMANAGER_PROCESS_RATE_MSECS;
...
/******************************************************************************
 * @function ButtonManagerDynamic_CreateButtonGpio
 *
 ...
 *
 *****************************************************************************/
BTNENUM ButtonManagerDynamic_CreateButtonGpio( GPIOPORT ePort, U8 nPin, BOOL bReleaseEnb, BOOL bPressEnb, BOOL bRptEnb, BOOL bShortEnb, BOOL bMedEnb, BOOL bLongEnb, BOOL bTglEnb, PVBTNMNGRCBFUNC pvCallback )
{
  GPIOENUM  eGpioEnum;
  BTNENUM   eBtnEnum;

  // create the GPIO
  eGpioEnum = Gpio_Configure( ePort, nPin, GPIO_MODE_INPUT_PULLUP, OFF, GPIO_FUNCMUX_A, ON );

  // now create the button
  eBtnEnum = ButtonManagerDynamic_CreateButton( eGpioEnum, bReleaseEnb, bPressEnb, bRptEnb, bShortEnb, bMedEnb, bLongEnb, bTglEnb, pvCallback );
...
BTNENUM ButtonManagerDynamic_CreateButton( GPIOENUM eGpioEnum, BOOL bReleaseEnb, BOOL bPressEnb, BOOL bRptEnb, BOOL bShortEnb, BOOL bMedEnb, BOOL bLongEnb, BOOL bTglEnb, PVBTNMNGRCBFUNC pvCallback )
{
  ...
  if (( ptNewBtn = malloc( BTNCTL_SIZE )) != NULL )
  {
    ...
    ptNewBtn->tEventFlags.bReleaseEnable    = bReleaseEnb;
    ptNewBtn->tEventFlags.bPressEnable      = bPressEnb;
    ptNewBtn->tEventFlags.bRepeatEnable     = bRptEnb;
    ptNewBtn->tEventFlags.bShortHoldEnable  = bShortEnb;
    ptNewBtn->tEventFlags.bMediumHoldEnable = bMedEnb;
    ptNewBtn->tEventFlags.bLongHoldEnable   = bLongEnb;
    ptNewBtn->tEventFlags.bToggleEnable     = bTglEnb;
    ...
    switch( ptCurBtn->eState )
    {
      ...
      case BTN_STATE_RELEASED :
        ...
        if ( bKeyState )
        {
          ...
          ptCurBtn->wDelayCounts = tParams.wDebounceTimeCounts;
      ...
      case BTN_STATE_DEBOUNCE :
        ...
        if ( bKeyState )
        {
          ...
          ptCurBtn->wDelayCounts = tParams.wRepeatDelayCounts;
      ...
      case BTN_STATE_PRESSED :
        ...
        if ( bKeyState )
        {
          if ( --ptCurBtn->wDelayCounts == 0 )
          {
            ...
            ptCurBtn->wDelayCounts = tParams.wRepeatRateCounts;
          ...
          if (( ptCurBtn->wHoldCounts >= tParams.wShortHoldTimeCounts ) & ( !ptCurBtn->tHoldDets.bShort ))
          {
            // mark it
          ...
          if (( ptCurBtn->wHoldCounts >= tParams.wMediumHoldTimeCounts ) & ( !ptCurBtn->tHoldDets.bMedium ))
          {
            // mark it
          ...
          // check for long hold
          if (( ptCurBtn->wHoldCounts >= tParams.wLongHoldTimeCounts ) && ( !ptCurBtn->tHoldDets.bLong ))
          {
            // mark it
          ...
          // check for stuck key
          if ( ptCurBtn->wHoldCounts >= tParams.wStuckTimeCounts )
          {
            // generate an event
...
static void PostEvent( BTNMNGREVENTS eEvent, PBTNCTL ptCtl )
{
      ...
      ( ptCtl->tEventFlags.bLongHoldEnable   && ( eEvent == BTNMNGR_EVENT_LONGHOLD )))
  {
    ...
    {
      // call the cllback
      ptCtl->pvCallback( ptCtl->eBtnEnum, eEvent );
    }
  }
}
