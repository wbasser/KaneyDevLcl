...
static  ALIGNED8 GMACDEVICE    tGmacDevice;
...
static void LowLevelInitialize( struct netif *ptNetIf )
{
  ...
  // initialize the PHY
  if ( PhyKSZ8081RNACA_Initialize(( PU32 )GMAC, 0 ) != GMAC_STATUS_OK )
  {
    // report error
  ...
  {
    // set auto negotiate
    if ( PhyKSZ8081RNACA_AutoNegotiate(( PU32 )GMAC, 0 ) != GMAC_STATUS_OK )
    {
      // report error
    ...
    {
      // establish link
      while( PhyKSZ8081RNACA_SetLink(( PU32 )GMAC, 0, 1 ) != GMAC_STATUS_OK )
      {
        // report error
