 ...
 *
 *****************************************************************************/
BOOL NormalCommands_Initialize( void)
{
  // return no errors
  return( FALSE );
}
...
static void	CmdUp( void)
{
  ...
  if (( g_ptDaliCtl->tDaliPrm.nFadeRate != 0 ) && ( g_ptDaliCtl->nActualDimLevel >= g_ptDaliCtl->tDaliPrm.nMinLevel ))
  {
    ...
    if ( g_ptDaliCtl->nActualDimLevel < g_ptDaliCtl->tDaliPrm.nMaxLevel )
    {
      ...
      g_ptDaliCtl->uFadeIncrement = auFadeRates[ g_ptDaliCtl->tDaliPrm.nFadeRate ];    

      // set the fade period to 200ms/set the fade direction
      g_ptDaliCtl->uFadePeriod = UP_DOWN_FADE_TIME;
      g_ptDaliCtl->eFadeDir = FADE_DIR_UP;
      g_ptDaliCtl->tFlags.bFadeRateReq = TRUE;
    }
    else
    {
...
static void	CmdStepUp( void)
{
  ...
  if (( g_ptDaliCtl->nActualDimLevel < g_ptDaliCtl->tDaliPrm.nMaxLevel ) && ( g_ptDaliCtl->nActualDimLevel >= g_ptDaliCtl->tDaliPrm.nMinLevel ))
  {
...
{
  // check to make sure we are not at max and that the lamp is on
  if (( g_ptDaliCtl->nActualDimLevel > g_ptDaliCtl->tDaliPrm.nMinLevel ) && ( g_ptDaliCtl->nActualDimLevel >= g_ptDaliCtl->tDaliPrm.nMinLevel ))
  {
...
static void	CmdEnbDAPCSeq( void)
{
  if ( g_ptDaliCtl->tFlags.bDAPCSequenceRunning == FALSE )
  {
    // set the fade time to 1
    g_ptDaliCtl->nCurFadeTime = g_ptDaliCtl->tDaliPrm.nFadeTime;
    g_ptDaliCtl->tDaliPrm.nFadeTime = 1;
  }
 
  // set the DAPC sequence running flag
  g_ptDaliCtl->tFlags.bDAPCSequenceRunning = TRUE;
}
...
static void	CmdScene( void)
{
  ...
  g_ptDaliCtl->nRequestedLevel = g_ptDaliCtl->tDaliPrm.anScenes[ g_ptDaliCtl->tCurMessage.tFields.nData & 0x0F ];
...
static void	CmdReset( void)
{
  ...
  g_ptDaliCtl->tFlags.bResetRequested = TRUE;
}
...
static void	CmdStoreLevelInDtr( void)
{
  ...
  g_ptDaliCtl->nDataTransReg = g_ptDaliCtl->nActualDimLevel;
}
...
static void	CmdStoreDtrMaxLvl( void)
{
  ...
  g_ptDaliCtl->tDaliPrm.nMaxLevel = g_ptDaliCtl->nDataTransReg;

  // now check for valid
  if ( g_ptDaliCtl->tDaliPrm.nMaxLevel <= g_ptDaliCtl->tDaliPrm.nMinLevel )
  {
    // set the max level to in level
    g_ptDaliCtl->tDaliPrm.nMaxLevel = g_ptDaliCtl->tDaliPrm.nMinLevel;
  }
  ...
  if ( g_ptDaliCtl->tDaliPrm.nMaxLevel == 0xFF )
  {
    // set the max level to 0xFE (254)
    g_ptDaliCtl->tDaliPrm.nMaxLevel = 0xFE;
  }

  // compare to reset state value
  DALIMessageHandler_GetDefaultValue( DALI_PARAM_MAX_LEVEL, &nValue );
  g_ptDaliCtl->tResetState.tBits.bMaxLevel = ( g_ptDaliCtl->tDaliPrm.nMaxLevel != nValue ) ? TRUE : FALSE;

  // update the NVM
  DALIMessageHandler_PutValue( DALI_PARAM_MAX_LEVEL, g_ptDaliCtl->tDaliPrm.nMaxLevel, FALSE );
}
...
static void	CmdStoreDtrMinLvl( void)
{
  ...
  // copy the DTR into the max level
  g_ptDaliCtl->tDaliPrm.nMinLevel = g_ptDaliCtl->nDataTransReg;

  // now check for valid
  if ( g_ptDaliCtl->tDaliPrm.nMinLevel >= g_ptDaliCtl->tDaliPrm.nMaxLevel )
  {
    // set the min level to max level
    g_ptDaliCtl->tDaliPrm.nMinLevel = g_ptDaliCtl->tDaliPrm.nMaxLevel;
  }
  
  //  check for min level less than physical minimum level
  if ( g_ptDaliCtl->tDaliPrm.nMinLevel < g_ptDaliCtl->nPhysicalMinLvl )
  {
    g_ptDaliCtl->tDaliPrm.nMinLevel = g_ptDaliCtl->nPhysicalMinLvl;
  }
  ...
  // compare to reset state value
  DALIMessageHandler_GetDefaultValue( DALI_PARAM_MIN_LEVEL, &nValue );
  g_ptDaliCtl->tResetState.tBits.bMinLevel = ( g_ptDaliCtl->tDaliPrm.nMinLevel != nValue ) ? TRUE : FALSE;

  // update the NVM
  DALIMessageHandler_PutValue( DALI_PARAM_MIN_LEVEL, g_ptDaliCtl->tDaliPrm.nMinLevel, FALSE );
}
...
static void	CmdStoreDtrSysFailLvl( void)
{
  ...
  // update the value
  g_ptDaliCtl->tDaliPrm.nSystemFailureLevel = g_ptDaliCtl->nDataTransReg;
  
  // compare to reset state value
  DALIMessageHandler_GetDefaultValue( DALI_PARAM_SYSFAIL_LEVEL, &nValue );
  g_ptDaliCtl->tResetState.tBits.bSystemFailureLevel = ( g_ptDaliCtl->tDaliPrm.nSystemFailureLevel != nValue ) ? TRUE : FALSE;
  
  // update NVM
  DALIMessageHandler_PutValue( DALI_PARAM_SYSFAIL_LEVEL, g_ptDaliCtl->tDaliPrm.nSystemFailureLevel, FALSE );
}
...
static void	CmdStoreDtrPwrOnLvl( void)
{
  ...
  // update the value
  g_ptDaliCtl->tDaliPrm.nPowerOnLevel = g_ptDaliCtl->nDataTransReg;

  // compare to reset state value
  DALIMessageHandler_GetDefaultValue( DALI_PARAM_POWERON_LEVEL, &nValue );
  g_ptDaliCtl->tResetState.tBits.bPowerOnLevel = ( g_ptDaliCtl->tDaliPrm.nPowerOnLevel != nValue ) ? TRUE : FALSE;

  // update NVM
  DALIMessageHandler_PutValue( DALI_PARAM_POWERON_LEVEL, g_ptDaliCtl->tDaliPrm.nPowerOnLevel, FALSE );
}
...
static void	CmdStoreDtrFadeTime( void)
{
  ...
  // update the value
  g_ptDaliCtl->tDaliPrm.nFadeTime = MIN( g_ptDaliCtl->nDataTransReg, 0x0F );

  // Compare to reset state value
  DALIMessageHandler_GetDefaultValue( DALI_PARAM_FADE_TIME, &nValue );
  g_ptDaliCtl->tResetState.tBits.bFadeTime = ( g_ptDaliCtl->tDaliPrm.nFadeTime != nValue ) ? TRUE : FALSE;

  // update the NVM
  DALIMessageHandler_PutValue( DALI_PARAM_FADE_TIME, g_ptDaliCtl->tDaliPrm.nFadeTime, FALSE );
}
...
static void	CmdStoreDtrFadeRate( void)
{
  ...
  // update the value
  g_ptDaliCtl->tDaliPrm.nFadeRate = MIN( g_ptDaliCtl->nDataTransReg, 0x0F );
  g_ptDaliCtl->tDaliPrm.nFadeRate = MAX( g_ptDaliCtl->tDaliPrm.nFadeRate, 1 );
  
  // Compare to reset state value
  DALIMessageHandler_GetDefaultValue( DALI_PARAM_FADE_RATE, &nValue );
  g_ptDaliCtl->tResetState.tBits.bFadeRate = ( g_ptDaliCtl->tDaliPrm.nFadeRate != nValue ) ? TRUE : FALSE;

  // update the NVM
  DALIMessageHandler_PutValue( DALI_PARAM_FADE_RATE, g_ptDaliCtl->tDaliPrm.nFadeRate, FALSE );
}
...
static void	CmdStoreDtrScene( void)
{
  ...
  nScene = g_ptDaliCtl->tCurMessage.tFields.nData & 0x0F;
  g_ptDaliCtl->tDaliPrm.anScenes[ nScene ] = g_ptDaliCtl->nDataTransReg;

  // set the appropriate scene bit in reset indicator structure
  if ( g_ptDaliCtl->tDaliPrm.anScenes[ nScene ] != SCENE_RESET )
  {
    // set scene modified bit
    g_ptDaliCtl->tResetState.tBits.wScene |= BIT( nScene );
  }
  else 
  {
  ...
  // update the NVM
  DALIMessageHandler_PutValue( DALI_PARAM_SCENE00 + nScene, g_ptDaliCtl->tDaliPrm.anScenes[ nScene ], FALSE );
}
...
static void	CmdRemoveScene( void)
{
  ...
  // get the scene index/store the DTR
  nScene = g_ptDaliCtl->tCurMessage.tFields.nData & 0x0F;
  g_ptDaliCtl->tDaliPrm.anScenes[ nScene ] = 0xFF;
...
static void	CmdAddToGroup( void)
{
  ...
  wGroupMask = BIT(( g_ptDaliCtl->tCurMessage.tFields.nData & 0x0F ));
  g_ptDaliCtl->tDaliPrm.tGroup.wValue |= wGroupMask;

  // set the reset bit to true - indicating groups are not in reset state
  g_ptDaliCtl->tResetState.tBits.bGroup = TRUE;

  // store the new group value
  DALIMessageHandler_PutValue( DALI_PARAM_GROUP0007, g_ptDaliCtl->tDaliPrm.tGroup.anValue[ LE_U16_LSB_IDX ], FALSE );
  DALIMessageHandler_PutValue( DALI_PARAM_GROUP0815, g_ptDaliCtl->tDaliPrm.tGroup.anValue[ LE_U16_MSB_IDX ], FALSE );
}
...
static void	CmdRemoveFromGroup( void)
{
  ...
  g_ptDaliCtl->tResetState.tBits.bGroup = ( g_ptDaliCtl->tDaliPrm.tGroup.wValue != 0 ) ? TRUE : FALSE;
...
static void	CmdStoreDtrAddress( void)
{
  ...
  if ( g_ptDaliCtl->nDataTransReg == 0xFF )
  {
    // set the address
    g_ptDaliCtl->tDaliPrm.nShortAddress = 0xFF;
  }
  else if (( g_ptDaliCtl->nDataTransReg <= 127 ) && (( g_ptDaliCtl->nDataTransReg & 0x01 ) != 0))
  {
    // set the address
    g_ptDaliCtl->tDaliPrm.nShortAddress = (( g_ptDaliCtl->nDataTransReg >> 1 ) & 0x3F );
  }
  ...
  DALIMessageHandler_PutValue( DALI_PARAM_SHORT_ADDR, g_ptDaliCtl->tDaliPrm.nShortAddress, FALSE );
}
...
static void	CmdEnableWriteMemory( void)
{
  ...
  g_ptDaliCtl->tFlags.bWriteMemoryEnabled = TRUE;
}
...
static void	CmdQryStatus( void)
{
  ...
  if ( ( g_ptDaliCtl->tResetState.uWord == 0 ) && ( ( g_ptDaliCtl->tDaliSts.nByte & STATUS_RESET_MASK ) == 0 ) )
  {
    g_ptDaliCtl->tDaliSts.tBits.bResetState = TRUE;
  }
  else
  {
    g_ptDaliCtl->tDaliSts.tBits.bResetState = FALSE;
  }
  
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliSts.nByte;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = ( g_ptDaliCtl->tDaliSts.tBits.bLampFailure ) ? DALI_RESPONSE_YES : DALI_RESPONSE_NO;
  g_ptDaliCtl->tFlags.bResponseRequested = ( g_ptDaliCtl->nResponseValue != DALI_RESPONSE_NO ) ? TRUE : FALSE;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = ( g_ptDaliCtl->tDaliSts.tBits.bLampOn == TRUE ) ? DALI_RESPONSE_YES : DALI_RESPONSE_NO;
  g_ptDaliCtl->tFlags.bResponseRequested = ( g_ptDaliCtl->nResponseValue != DALI_RESPONSE_NO ) ? TRUE : FALSE;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = ( g_ptDaliCtl->tDaliSts.tBits.bLimitError == TRUE ) ? DALI_RESPONSE_YES : DALI_RESPONSE_NO;
  g_ptDaliCtl->tFlags.bResponseRequested = ( g_ptDaliCtl->nResponseValue != DALI_RESPONSE_NO ) ? TRUE : FALSE;
}
...
static void	CmdQryResetState( void)
{
  ...
  g_ptDaliCtl->nResponseValue = ( g_ptDaliCtl->tDaliSts.tBits.bResetState == TRUE ) ? DALI_RESPONSE_YES : DALI_RESPONSE_NO;
  g_ptDaliCtl->tFlags.bResponseRequested = ( g_ptDaliCtl->nResponseValue != DALI_RESPONSE_NO ) ? TRUE : FALSE;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = ( g_ptDaliCtl->tDaliSts.tBits.bMissingShortAddr == TRUE ) ? DALI_RESPONSE_YES : DALI_RESPONSE_NO;
  g_ptDaliCtl->tFlags.bResponseRequested = ( g_ptDaliCtl->nResponseValue != DALI_RESPONSE_NO ) ? TRUE : FALSE;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = DALI_VERSION_NUMBER;
}
...
static void	CmdQryContentDtr( void)
{
  ...
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->nDataTransReg;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->nDeviceType;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->nPhysicalMinLvl;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = ( g_ptDaliCtl->tDaliSts.tBits.bPowerFailure == TRUE ) ? DALI_RESPONSE_YES : DALI_RESPONSE_NO;
  g_ptDaliCtl->tFlags.bResponseRequested = ( g_ptDaliCtl->nResponseValue != DALI_RESPONSE_NO ) ? TRUE : FALSE;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->nDataTransReg1;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->nDataTransReg2;
}
...
static void	CmdQryActualLvl( void)
{
  ...
  g_ptDaliCtl->nResponseValue = ( g_ptDaliCtl->tDaliSts.tBits.bLampFailure == TRUE ) ? 0xFF : g_ptDaliCtl->nActualDimLevel;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.nMaxLevel;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.nMinLevel;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.nPowerOnLevel;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.nSystemFailureLevel;
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.nFadeTime << 4;
  g_ptDaliCtl->nResponseValue |= ( g_ptDaliCtl->tDaliPrm.nFadeRate & 0x0F );
}
...
static void	CmdQrySceneLvl( void)
{
  ...
  nScene = g_ptDaliCtl->tCurMessage.tFields.nData & 0x0F;
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.anScenes[ nScene ];
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.tGroup.anValue[ LE_U16_LSB_IDX ];
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.tGroup.anValue[ LE_U16_MSB_IDX ];
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.tRandomAddr.anValue[ RANDOM_ADDR_H ];
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.tRandomAddr.anValue[ RANDOM_ADDR_M ];
}
...
{
  // set the return value
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.tRandomAddr.anValue[ RANDOM_ADDR_L ];
}
...
static void	CmdQryMemoryLoc( void)
{
  ...
  // read the parameter
  if ( DALIMessageHandler_ReadMemoryBlock( g_ptDaliCtl->nDataTransReg1, g_ptDaliCtl->nDataTransReg, &nValue ))
  {
    //set the response value to the content of the memory block
    g_ptDaliCtl->nResponseValue = nValue;

    // increment the address
    g_ptDaliCtl->nDataTransReg++;
  ...
  if ( DALIMessageHandler_ReadMemoryBlock( g_ptDaliCtl->nDataTransReg1, g_ptDaliCtl->nDataTransReg, &nValue ))
  {
    ...
    g_ptDaliCtl->nDataTransReg2 = nValue;
  }
}
...
{
  // Log is 0 Lin is 1
  if (( g_ptDaliCtl->nDataTransReg == 0x01 ) || ( g_ptDaliCtl->nDataTransReg == 0x00 ))
  {  
    g_ptDaliCtl->tDaliPrm.tOpMode.tFields.bDimCurve = ( BOOL )g_ptDaliCtl->nDataTransReg;
    DALIMessageHandler_PutValue( DALI_PARAM_OPMODE, g_ptDaliCtl->tDaliPrm.tOpMode.nOpMode, FALSE );
  }
}
...
static void CmdStoreDtrFastFadetime( void)    
{
  ...
  if (( g_ptDaliCtl->nDataTransReg == 0 ) || 
      (( g_ptDaliCtl->nDataTransReg >= g_ptDaliCtl->nMinFastFadeTime ) && ( g_ptDaliCtl->nDataTransReg <= 27 )))
  {
     // update the value
    g_ptDaliCtl->tDaliPrm.nFastFadeTime = g_ptDaliCtl->nDataTransReg;
  }
  // if DTR is in between 0 and Minimum Fast Fade time then the Fast Fade Time shall be the Minimum Fast Fade Time
  else if (( g_ptDaliCtl->nDataTransReg > 0 ) && ( g_ptDaliCtl->nDataTransReg < g_ptDaliCtl->nMinFastFadeTime ))
  {
    // update the value
    g_ptDaliCtl->tDaliPrm.nFastFadeTime = g_ptDaliCtl->nMinFastFadeTime;
  }
  // if DTR is greater than 27 then Fast Fade Time shall be 27
  else if ( g_ptDaliCtl->nDataTransReg > 27 )
  {
    // update the value
    g_ptDaliCtl->tDaliPrm.nFastFadeTime = 27;
  }

  // update the NVM
  DALIMessageHandler_PutValue( DALI_PARAM_FASTFADETIME, g_ptDaliCtl->tDaliPrm.nFastFadeTime, FALSE );
}
...
static void CmdQryGearType( void)             
{
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tGearType.nGearType; 
...
{
  // Log is 0 Lin is 1
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.tOpMode.tFields.bDimCurve;
}
...
{
  // return the op types
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tPossibleOpTypes.nPossibleOpTypes;
}
...
static void CmdQryFeatures( void)          
{
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tFeatures.nFeatures;
}
...
static void CmdQryFailiureStatus( void)      
{
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.tFailureStatus.nFailureStatus;
}
...
static void CmdQryOpMode( void)               
{
   g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.tOpMode.nOpMode;
}
...
static void CmdQryFastFadeTime( void)         
{
   g_ptDaliCtl->nResponseValue = g_ptDaliCtl->tDaliPrm.nFastFadeTime;
}
...
{
  g_ptDaliCtl->nResponseValue = g_ptDaliCtl->nMinFastFadeTime;
}
