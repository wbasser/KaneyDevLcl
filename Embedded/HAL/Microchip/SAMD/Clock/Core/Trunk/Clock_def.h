...
#define CLOCKOSCDEF32KINT( ondemand, standby, out1kenb, out32kenb ) \
  { \
...
#define CLOCKOSCDEF8MHZ( ondemand, standby, outdiv ) \
  { \
...
#define CLOCKOSCDEFDFLLOPEN( mult, coarse, fine, coarsestep, finestep, ondemand, usbrecover, quicklock, tracklock, lockonwakeup, chillcycle, overwriteenb ) \
  { \
    ...
    { \
      ...
      .bDisableChillCycle = chillcycle, \
      .bOverwriteCal = overwriteenb, \
    }, \
  }

/// define the helper macro to create an DFLL oscillator open loop mode
#define CLOCKOSCDEFDFLLCLOSED( mult, coarse, fine, ondemand, usbrecover, quicklock, tracklock, lockonwakeup, chillcycle, overwriteenb ) \
  { \
    .eClockOsc = CLOCK_OSC_DFLL, \
...
typedef struct _CLOCKOSCDEF
{
  ...
  struct  
  {
    ...
    BOOL            bDisableChillCycle  : 1;  ///< disable chill cycle
