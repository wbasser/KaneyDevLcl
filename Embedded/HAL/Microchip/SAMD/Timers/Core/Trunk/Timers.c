...
//    uParam = ptParam->uValue;
    ...
    switch( eAction )
    {
        ...
        if ( ptDef->eChannel < TIMER_CHAN_3 )
        {
          ...
          switch( ptDef->tModes.eTcc )
          {
            case TIMER_TCCMODE_PWMSING :
              ...
              {
                // compute the period
                uTemp = ( uDivider / ptParam->uValue ) / GetPrescaleValue( ptDef->ePrescale );
              }
      ...
      case TIMER_IOCTL_SETCOMPAREPCT :
        ...
        {
          // compute the divider
          if( ptDef->tModes.eTc == TIMER_TCMODE_MPWM )
          {
            uDivider = tCtl.tBasePtrs.ptTc->COUNT16.CC[0].reg;
          }
          else
          {
            uDivider = 65535;
      ...
      case TIMER_IOCTL_SETPPERIOD50 :
        if ( ptDef->eChannel < TIMER_CHAN_3 )
        {
          ...
          switch( ptDef->tModes.eTcc )
          {
            ...
            // write it
            bState = tCtl.tBasePtrs.ptTcc->CTRLA.bit.ENABLE;
...
static void CommonTccInterruptHandler( TIMERENUM eTimer, Tcc* ptTcc )
{
  ...
  {
    ptTcc->INTFLAG.bit.OVF = tIntFlag.bit.OVF;
    eEvent = TIMER_CBEVENT_TMO;
  }
  else if ( tIntFlag.bit.MC0 & ptTcc->INTENSET.bit.MC0 )
  {
    ptTcc->INTFLAG.bit.MC0 = tIntFlag.bit.MC0;
    uCapValue = ptTcc->CC[0].reg; 
    eEvent = TIMER_CBEVENT_CAPTURE;
  }
  else if ( tIntFlag.bit.MC1 & ptTcc->INTENSET.bit.MC1 )
  {
    ptTcc->INTFLAG.bit.MC1 = tIntFlag.bit.MC1;
    uCapValue = ptTcc->CC[ 1 ].reg;
    eEvent = TIMER_CBEVENT_CAPTURE;
  }
  else if ( tIntFlag.bit.MC2 & ptTcc->INTENSET.bit.MC2 )
  {
    ptTcc->INTFLAG.bit.MC2 = tIntFlag.bit.MC2;
    uCapValue = ptTcc->CC[ 2 ].reg;
    eEvent = TIMER_CBEVENT_CAPTURE;
  }
  else if ( tIntFlag.bit.MC1 & ptTcc->INTENSET.bit.MC3 )
  {
    ptTcc->INTFLAG.bit.MC3 = tIntFlag.bit.MC3;
    uCapValue = ptTcc->CC[ 3 ].reg;
    eEvent = TIMER_CBEVENT_CAPTURE;
  }
  else if ( tIntFlag.bit.CNT & ptTcc->INTENSET.bit.CNT )
  {
    ptTcc->INTFLAG.bit.CNT = tIntFlag.bit.CNT;
  ...
  {
    // call the callback
    pvCallback( eEvent, eCmpCap, uCapValue );
  }
}
