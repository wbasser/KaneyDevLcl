...
BOOL Event_Initialize( void )
{
  ...
  PowerManager_DisableEnablePeriphC( PM_APBCMASK_EVSYS, ON );
}
 ...
 *
 *****************************************************************************/
void Event_Create( EVENTCHAN eChannel, EVENTGEN eGenerator, EVENTUSERMUX eUserMux, EVENTEDGE eEdge )
{
  EVSYS_CHANNEL_Type  tChan;
  U8                  nTemp;

  // enable the clock
  Clock_PeriphEnable( CLOCK_MUXID_EVSYSCH0 + eChannel, CLOCK_GENID_0 ); 
