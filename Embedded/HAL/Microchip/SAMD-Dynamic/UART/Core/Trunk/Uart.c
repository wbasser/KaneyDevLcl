    ...
    if (( ptCtl->tTxBuf.pnBuffer = malloc( ptDef->wTxBufSize )) != NULL )
    {
      ...
      #endif // SYSTEMDEFINE_OS_SELECTON != SYSTEMDEFINE_OS_MINIMAL
      {
        ...
        if (( wBaudRate = ComputeBaudrate( ptDef )) != 0 )
        {
          ...
          // check for RTS/CTS
          if ( ptDef->eTxdPad == UART_TXDPAD_2 )
          {
            // configure the RTS/CTS lpins
            Gpio_Configure( ptDef->eDevPort, ptDef->nRtsPin, GPIO_MODE_OUTPUT_INPDSB, OFF, ptDef->eDevMux, OFF );
            Gpio_Configure( ptDef->eDevPort, ptDef->nCtsPin, GPIO_MODE_INPUT,         OFF, ptDef->eDevMux, OFF );
          }
          ...
          ptCtl->ptUsart->CTRLA = tCtrlA;
          while( ptCtl->ptUsart->SYNCBUSY.bit.CTRLB & SERCOM_USART_SYNCBUSY_CTRLB );
          ptCtl->ptUsart->CTRLB = tCtrlB;

          // set up the interrupts
          if ( ptDef->tEventFlags.bTxCmp )
          {
            ptCtl->ptUsart->INTENSET.bit.TXC = OFF;
          }
          if ( ptDef->tEventFlags.bRxChar )
          {
            ptCtl->ptUsart->INTENSET.bit.RXC = ON;
          }
          if ( ptDef->tEventFlags.bRxErr )
          {
            ptCtl->ptUsart->INTENSET.bit.ERROR = ON;
          }
          if ( ptDef->tEventFlags.bBrkDet )
          {
            ptCtl->ptUsart->INTENSET.bit.RXBRK = ON;
          }

          // clear the inteerupts
          ptCtl->ptUsart->INTENCLR.reg = 0xFF;
