...
U32 Gpio_Configure( GPIOPORT ePort, U8 nPin, GPIOMODE eMode, BOOL bHiDriveEnb, GPIOFUNCMUX eFunc, BOOL bInvert )
{
  ...
  tPinCfg.bit.PULLEN = (( eMode == GPIO_MODE_INPUT_PULLDN ) || ( eMode == GPIO_MODE_INPUT_PULLUP )) ? TRUE : FALSE;
  ...
  switch( eMode )
  {
    ...
    case GPIO_MODE_INPUT_PULLUP :
      ...
      ptPort->OUTSET.reg = ( eMode == GPIO_MODE_INPUT_PULLUP ) ? uMask : 0;
      break;
 ...
 *****************************************************************************/
#ifdef _SAMD11_
  GPIOENUM 	Gpio_ConfigureIRQ( GPIOPORT ePort, U8 nPin, U8 nIrqNum, GPIOIRQSENSE eSense, BOOL bPullupEnable, BOOL bFilterEnable, BOOL bWakeup, BOOL bInitOn, BOOL bEvent, PVGPIOIRQCALLBACK pvCallback )
#else
...
#endif
{
  ...
  // enable the EIC
  Clock_PeriphEnable( CLOCK_MUXID_EIC, CLOCK_GENID_0 ); 
  ...
  tPinCfg.bit.PULLEN = ( bPullupEnable ) ? ON : OFF;
  ptPort->PINCFG[ nPin ] = tPinCfg;
