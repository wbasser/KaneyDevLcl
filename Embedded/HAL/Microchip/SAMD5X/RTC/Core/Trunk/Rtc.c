...
#define PRESCALE_VALUE          ( RTC_MODE2_CTRLA_PRESCALER_DIV32_Val	)
...
{
  RTC_MODE2_CTRLA_Type tMode2CtrlA;

  // reset the device
  RTC->MODE2.CTRLA.bit.SWRST = ON;
  while( RTC->MODE2.SYNCBUSY.bit.SWRST );
	
  // Write the local control register structure
  tMode2CtrlA.reg = RTC_MODE2_CTRLA_MODE( 2 );
  tMode2CtrlA.reg |= RTC_MODE2_CTRLA_PRESCALER( PRESCALE_VALUE );
	
  // Write the hardware register
  RTC->MODE2.CTRLA = tMode2CtrlA;
