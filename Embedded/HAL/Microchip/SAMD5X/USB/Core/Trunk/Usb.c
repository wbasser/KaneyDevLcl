...
#define NVM_READ_CAL(cal) \
    ((*(( PU32 )NVMCTRL_SW0 + NVM_##cal##_POS / 32)) >> (NVM_##cal##_POS % 32)) & ((1 << NVM_##cal##_SIZE) - 1)
...
BOOL Usb_Initialize( void )
{
  ...
  for ( nTemp = 0; nTemp < USB_NUM_ENDPOINTS; nTemp++ )
  {
    Usb_ResetEndpoint( nTemp, USB_ENDPOINT_IN );
    Usb_ResetEndpoint( nTemp, USB_ENDPOINT_OUT );
  }

  // enable the interrupts
  for ( nTemp = 0; nTemp < 4; nTemp++ )
  {
    NVIC_EnableIRQ( USB_0_IRQn + nTemp );
  }
 ...
 * @brief close the USB
 *
 * This function will close the USB
 *
 * @return  TRUE if errors, FALSE if none
 *
 *****************************************************************************/
void Usb_Close( void )
{
  ...
  for ( nTemp = 0; nTemp < 4; nTemp++ )
  {
    NVIC_DisableIRQ( USB_0_IRQn + nTemp );
  }
}
...
/******************************************************************************
 * @function CommonIrqHandler
 *
 * @brief USB common interrupt handler
 *
 * This function will process any USB intterrupts
 *
 *****************************************************************************/
void CommonIrqHandler( void )
{
