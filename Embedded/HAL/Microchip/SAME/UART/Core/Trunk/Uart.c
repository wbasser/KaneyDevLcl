...
BOOL Uart_Initialize( void )
{
  ...
  for ( eDev = 0; eDev < UART_DEV_ENUM_MAX; eDev++ )
  {
    ...
    if (( wBaudRate = ComputeBaudrate( ptDef->uBaudRate )) != 0 )
    {
      ...
      #if ( SYSTEMDEFINE_OS_SELECTION == SYSTEMDEFINE_OS_FREERTOS )
        NVIC_SetPriority( ptCtl->uIrqEnum, configMAX_SYSCALL_INTERRUPT_PRIORITY - 1 );
      #endif // SYSTEMDEFINE_OS_SELECTION
...
UARTERR Uart_Close( UARTDEVENUM eDev )
{
  ...
  if ( eDev < UART_DEV_ENUM_MAX )
  {
    ...
    // disable the USART
    ptCtl->ptUart->UART_CR &= ~( UART_CR_RXEN | UART_CR_TXEN );
  }
  else
  {
    // illegal device
 ...
 *
 * @brief interrupt UART3 handler
 *
 * This function handles the interrupts from UART3
 *
 *****************************************************************************/
 ...
 *
 * @brief interrupt UART4 handler
 *
 * This function handles the interrupts from UART4
 *
 *****************************************************************************/
...
static void GetUartPointer( UARTCHAN eChan, PLCLCTL ptCtl )
{
  switch( eChan )
  {
    case UART_CHAN_0 :
      ptCtl->ptUart = UART0;
      ptCtl->uIrqEnum = UART0_IRQn; 
      break;
      
    case UART_CHAN_1 :
      ptCtl->ptUart = UART1;
      ptCtl->uIrqEnum = UART1_IRQn;
      break;
    
    case UART_CHAN_2 :
      ptCtl->ptUart = UART2;
      ptCtl->uIrqEnum = UART2_IRQn;
      break;
    
    case UART_CHAN_3 :
      ptCtl->ptUart = UART3;
      ptCtl->uIrqEnum = UART3_IRQn;
      break;
    
    case UART_CHAN_4 :
      ptCtl->ptUart = UART4;
      ptCtl->uIrqEnum = UART4_IRQn;
      break;
